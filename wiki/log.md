...
## [2026-09-11] decision + build | Leaving Lovable (reaffirmed by Fabien), exit implemented the same day

- Decision reversed from the morning's "defer the full exit": Fabien reaffirmed twice; the database was still empty (seed only), which makes the move nearly free now and costly after launch. Costs confirmed: 0 €/month fixed at launch (Supabase Free, Vercel Hobby), Claude API ≈ 3 c€ per submitted problem, Lovable subscription can be cancelled.
- Created Supabase project `ouch` (ref `mywbvjaitfqclenrvsdn`, eu-west-3) in Fabien's org (cost shown by Supabase: 0 $/month); replayed the profiles + shared-persistence migrations; seeded the 38 cards (counters at zero).
- Code (branch `claude/persistence-spec` of `fix-it-karma`): standard Vite config (TanStack Start + Nitro `vercel` preset), native `supabase.auth.signInWithOAuth` for Google, Lovable auth broker / preview storage / cron auth / telemetry removed, qualification and duplicate detection on the Anthropic SDK (`claude-opus-5`, structured outputs, low effort, cached system prompt), AI illustration function dropped (unused), npm lockfile replaces the bun lockfile pinned to Lovable's registry, `.env` on the new project (public values only), `.env.example`, `docs/deploy.md`.
- Verified: typecheck, Vercel build (`.vercel/output` produced), dev server boots and serves `/`, `/terminal`, `/communaute/independants`. Not verifiable from the sandbox: OAuth round-trip and AI calls (need the secrets and network).
- Fabien's part (≈30 min, `docs/deploy.md` §1): Anthropic API key, Google OAuth client, Vercel import with the 6 env vars. Then two-device test on the Vercel URL, then disconnect Lovable.

## [2026-09-11] note | PR #1 merged by Fabien while the exit was being built; main was briefly broken

- Fabien opened and merged PR #1 (`claude/persistence-spec` → `main`) at 15:07 UTC, after merging `main` into the branch himself. The Lovable agent had pushed 4 commits to `main` in parallel (flippable swipe card, "review my votes" mode, sign-in on the published step).
- The manual merge left `main` unbuildable: an unclosed JSX element in `swipe-deck.tsx` and a reference to the removed `votes` field. Repaired on the rebased branch (flippable card restored, `voteDirectionFor` added to the provider). Lesson for the plan's "one driver on the code at a time" rule: it was violated the very first day, and cost an hour. Now moot: Lovable is being disconnected.
- Known limitation: the Lovable "review my votes" mode lets a visitor re-swipe cards; with insert-only votes and one vote per device, a re-swipe changes nothing server-side (the UI shows the previous choice). To be decided later if changing a vote matters.
