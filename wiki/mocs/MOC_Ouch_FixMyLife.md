...
- **2026-09-12 read of `main` (`924f93f`)**: `/entite/:slug` shows 3 cards then an ungated « Vue détaillée Maker » block with invented `devWeeks` (to remove); **no `/devenir-maker` route exists** (earlier pages citing it are stale; maker email capture has no surface yet); the qualifier drops the name of any organisation absent from the registry. Fixes sequenced in the [entity pages vs Maker space](../syntheses/strategy/2026-09-12_entity-pages-vs-maker-space.md) page; **shipped on `main` 2026-09-12 (PR #15)**: folded entity page without the maker block, `entity_page_visit`, private `entity_candidates` queue (migration applied live).
- **2026-09-13, auto-prepared entity registry** ([PR #22](https://github.com/xeonfab/fix-it-karma/pull/22), **merged 2026-09-12 08:28 UTC**): the registry moved to the `entities` table (migration applied live, 12 seed entities published; `entities.ts` keeps the seed as offline fallback). Daily cron `/api/entites/cron` (06:00 UTC, `CRON_SECRET`) publishes entities past their 48 h veto and prepares new candidates over the threshold (2 cards or 1 card ≥10 🔥) with the model (alias grouping, `existing_slug`, persons/categories refused and remembered). `/admin/entites` (Google login, `ADMIN_EMAILS`): publish / reject / run now. Notification by Resend or webhook. After merge Fabien sets `CRON_SECRET` and `ADMIN_EMAILS` in Vercel (`docs/deploy.md` §1.d).
...
- ~~Can an entity page be a paid teaser, and who creates an entity page?~~ **Settled 2026-09-12 (panel), confirmed by Fabien the same day**: no paywall on cards ever, the Terminal sells intent data after the signal; creation is never claimed by the entity. **Amended 2026-09-12 evening and built 2026-09-13**: creation is *auto-prepared* (model groups the spellings and drafts the entity, non-public « ready » state), then published by Fabien in one click or after a 48 h veto window; never a fully automatic publication without that window.
