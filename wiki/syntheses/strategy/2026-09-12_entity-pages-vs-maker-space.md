...
| Fabien + `ouch-editeur-cartes` | Sunday: read the candidate query, promote per the threshold; note the weekly count of candidates (trigger for the auto-prepared path: >5 on two consecutive Sundays) | from 2026-10-12 | open |
| `ouch-cto` | Auto-prepared entity creation with 48 h veto (registry in a table, alias grouping proposed by the model, non-public ready state, notification) | built at once at Fabien's request (« crée déjà le chemin auto-préparé ») | **done 2026-09-13** — PR `claude/entity-auto-prepare` on fix-it-karma; migration applied live (12 seed entities published) |
| Wiki | Correct the stale `/devenir-maker` mentions (full context page, launch plan) | 2026-09-12 | **done in this page and the MOC** |

## Follow-up decision (2026-09-12, evening) — automation of entity creation

Fabien challenged the manual step: the thresholds are mechanical, why click? Settled after discussion:

- **What the threshold cannot do**: it measures popularity, not legitimacy. The legal grid's three checks (real public organisation, canonical name, attributable and sourced friction) need a look, because a wrong entity page is public, indexed, and never deleted (hard rule 6). Two cards from one author would pass the threshold for a person's trade name or a product.
- **Alias grouping** = recognising that « Shine », « shine.fr » and « Shine banque » are one organisation, counting their cards together, and storing the spellings as `aliases` so later cards link automatically. Today Fabien does it by reading query 15; the model can propose it, a human confirms (« Qonto » / « Qonto Pro » yes, « Shine » / « Shine Assurances » maybe not).
- **Decision (validated by Fabien)**: creation stays manual now (0–3 candidates a week, five minutes on Sunday). When the queue exceeds **5 candidates a week on two consecutive Sundays**, build the **auto-prepared path**: a candidate over the threshold is prepared automatically (slug, grouped aliases proposed by the model, type and sector guessed, description, sources of its cards), lands as a **non-public ready entity**, Fabien is notified and validates in one click, or it publishes itself after a **48 h veto window**. Never a fully automatic publication without that window. This also requires moving the registry from `entities.ts` to a database table. **Update 2026-09-13**: Fabien asked to build it right away rather than wait for the trigger; shipped as the `entities` table + daily cron + `/admin/entites` (see log).
