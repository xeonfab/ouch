...
## [2026-09-13] repair | `wiki/log.md` and `wiki/index.md` restored after the PR #1 merge dropped entries

- The merge of PR #1 (`22b1d9b`, branch `claude/concept-test-messages`) resolved `wiki/log.md` and `wiki/index.md` by taking the older branch's version: 20 log entries from 2026-09-11/12 (freelance push, Lovable exit, Vercel deployment, field check, PR #4–#8) and five Strategy index rows (launch plan, rollout playbook, channel plan, channel map, team audit) disappeared from `main`, while the pages themselves stayed.
- Rebuilt both as the union of the two versions (`a34feb9` + the four entries only on the PR #1 branch), in date order. Backlink blocks regenerated with `wiki/backlinks.py`.

## [2026-09-13] deliverable | Facebook finance group — first group post (pre-flight + copy)

- Trigger: Fabien wants to post in a Facebook group described as a finance community (share link unreachable from the sandbox: group name, audience and rules unverified).
