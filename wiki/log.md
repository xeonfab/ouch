...
## [2026-09-12] decision | Panel: entity pages vs the Maker space

- Fabien asked the whole team to settle the design strategy of maker modules vs entity pages, whether a paid teaser on entity pages is legally viable, and how an entity page is created (claimed by the entity, automatic, or via hashtags for unregistered organisations). Raw note saved in `raw/transcripts/2026-09-12_reflexion-pages-entite-espace-maker.md`.
- Panel (5 seats + legal, CFO, UX, card editor) → `wiki/syntheses/strategy/2026-09-12_entity-pages-vs-maker-space.md`. Decisions: **cards are public, intent data is sold** (no paywall on the entity page ever; per-card opt-in access, trends, demographics, export, alerts go to the Terminal Pro after the Cercle 2 signal, D4 unchanged); **entity creation is editorial** (no claim / « officialiser » flow, hard rule 4; no auto-creation, legal registry rule); **the bridge for unregistered organisations is a private `entity_candidate` field** filled by the qualifier, not the hashtag (hashtags stay context-only). Legal also requires a « signaler cette carte » link and a consent update before any opt-in access is sold.
...
- Visual check done locally (`vite preview` + mocked Supabase rows, 420 px): header number, 3 open cards, fold, inline vote on a folded row, Terminal strip, no maker block, no console error; the current entity's tag is no longer repeated on folded rows (follow-up commit). The Vercel preview is unreachable from the sandbox. Terminal's « Dev estimé » (`devWeeks`) is still an invented number, out of this PR's scope; to remove in a later pass.
- Watch-out: another session opened `claude/lead-prompt-everywhere` at the same time (opt-in prompt after every positive vote, touches the list rows); whichever PR merges second must be rebased and re-typechecked (CTO rule: one PR at a time on shared components).

## [2026-09-12] merge | PR #15: main merged twice, metrics.sql reconciled
...
## [2026-09-12] pr | PR #22 (auto-prepared registry) green and mergeable after two merges of main
