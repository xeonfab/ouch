...
## [2026-09-12] repair | Merge 8c5e1ec had dropped 18 log entries and 5 index rows

- Cause: the merge of `claude/install-data-backup-gixofy` into `claude/concept-test-messages-v4my66` kept the older branch's `wiki/log.md` and `wiki/index.md`, erasing the 2026-09-11/12 entries (launch plan, rollout playbook, deck, channel plan, skills audit, Vercel deploy, field check, channel map) and the 5 Strategy index rows pointing to those pages
- Restored: `wiki/log.md` as the union of both branches (28 entries), `wiki/index.md` Strategy table (rollout playbook, skills audit, channel map v0, channel communication plan, 90-day plan), backlinks regenerated
- Trigger: Fabien asked "how do we launch to the freelance community, on which channel, what is the strategy?" — answered from the wiki (no new page: the [90-day plan](syntheses/strategy/2026-09-11_launch-plan-90-days.md), [rollout playbook](syntheses/strategy/2026-09-11_community-rollout-playbook.md), [channel plan](syntheses/strategy/2026-09-11_freelance-channel-communication-plan.md) and [channel map](syntheses/strategy/2026-09-12_freelance-channel-map.md) already cover it)

## [2026-09-12] export | Freelance launch strategy and channel messages copied to Notion
...
- Built in `fix-it-karma`, PR #31: cron `/api/recolte/cron` (daily since PR #32; the 6 h schedule made the Vercel Hobby deploy fail, 404 on `/admin/recolte`), table `harvest_candidates` (service role only), `harvest-reddit.server.ts` (Reddit OAuth or public JSON, filters, qualification via the shared `qualifyStory()`, duplicate check via `findDuplicate()`, reply texts), admin page `/admin/recolte`, enum value `harvest` on `problem_source`; qualification and duplicate logic extracted to `qualify.server.ts` / `duplicate.server.ts`; tsc, eslint, prettier, build green
...
## 2026-09-14 — fix: Reddit harvest cron made daily (Vercel Hobby limit)

- After PRs #30 and #31 merged, `/admin/recolte` returned 404 in production while CI was green on `main` and the route was in `routeTree.gen.ts`. Most likely cause: the `0 */6 * * *` cron in `vercel.json`; Vercel Hobby only accepts daily crons, so the production deploy failed and the site stayed on the previous build. Could not be confirmed from the sandbox (vercel.com egress blocked).
- Fix in `fix-it-karma` [PR #32](https://github.com/xeonfab/fix-it-karma/pull/32): cron `0 7 * * *`, admin page wording « chaque matin », note in `docs/deploy.md`. Extra runs via « Lancer une récolte maintenant ».
- Wiki: [reddit-harvest-pipeline](syntheses/projects/2026-09-14_reddit-harvest-pipeline.md) and the [channel communication plan](syntheses/strategy/2026-09-11_freelance-channel-communication-plan.md) now say daily. Notion « Com freelances » §13.1 and §13.5 updated.
