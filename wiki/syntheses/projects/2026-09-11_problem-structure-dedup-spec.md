...
- Voting and being notified are two intents (PR #16 then #20, `LeadPromptProvider`). Every positive vote, on any surface, is followed by the same offer: the skippable Google / email prompt on the first vote of a visit, then a toast with a one-tap « Préviens-moi 🔔 » per card (no form once the email is known), plus a bell on voted rows and cards to ask later. Never a silent registration: « N en attente d'une solution » counts explicit requests only. A vote surface without the offer is a lead leak.
